...
function CountlyBulk(conf) {
    ...
    if (!conf.app_key) {
        cc.log("app_key is missing");
        return;
    }

    if (!conf.url) {
        cc.log("url is missing");
        return;
    }
    ...
    **/
    this.add_request = function(query) {
        if (cluster.isMaster) {
            if (!query.device_id) {
                cc.log("device_id is missing", query);
                return;
            }
            ...
            if ((query.timestamp + "").length !== 13 && (query.timestamp + "").length !== 10) {
                cc.log("incorrect timestamp format", query);
            }
    ...
    this.add_bulk_request = function(requests) {
        if (cluster.isMaster) {
            ...
            for (var i in requests) {
                ...
                query.hour = query.hour || date.getHours();
                query.dow = query.dow || date.getDay();
                cc.log("adding bulk request", query);
                requestQueue.push(query);
            }
            storeSet("cly_req_queue", requestQueue);
        }
    ...
    */
    function heartBeat() {
        ...
        if (bulkQueue.length > 0 && readyToProcess && cc.getTimestamp() > failTimeout) {
            ...
            makeRequest(params, function(err, res) {
    ...
    var readFile = function(key) {
        ...
        if (conf.persist_queue) {
            ...
            try {
                ...
                fs.writeFile(path.resolve(__dirname, conf.storage_path + "__" + key + "." + cc.getTimestamp() + Math.random() + ".json"), data, function() {});
