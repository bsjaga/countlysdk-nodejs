 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            Countly.debug = conf.debug || Countly.debug || false;
            Countly.app_key = conf.app_key || Countly.app_key || null;
            Countly.url = stripTrailingSlash(conf.url || Countly.url || "");
    ...
    var updateConsent = function() {
        ...
        consentTimer = setTimeout(function() {
            if (Object.keys(syncConsents).length) {
                ...
                log("Consent removal request has been sent to the queue.");
    ...
    */
    function recordError(err, nonfatal, segments) {
        if (Countly.check_consent("crashes") && err) {
            ...
            var m = getMetrics();
            var ob = {_os: m._os, _os_version: m._os_version, _error: error, _app_version: m._app_version, _run: getTimestamp() - startTime};
