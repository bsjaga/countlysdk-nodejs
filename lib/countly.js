 ...
 */
    Countly.init = function(conf) {
        if (!inited) {
            ...
            maxCrashLogs = conf.max_logs || Countly.max_logs || maxCrashLogs;
            ...
            if (Countly.url === "") {
                cc.log("Please provide server URL");
            }
            else {
                cc.log("Countly initialized");
                if (cluster.isMaster) {
            ...
            for (var i in features) {
                if (!consents[i]) {
                    if (typeof features[i] === "string") {
                        ...
                        cc.log("Incorrect feature list for " + i + " value: " + features[i]);
                    }
                }
                else {
                    cc.log("Feature name " + i + " is already reserved");
                }
            }
        }
        else {
            cc.log("Incorrect features: " + features);
        }
    };
    ...
    */
    Countly.add_consent = function(feature) {
        cc.log("Adding consent for " + feature);
    ...
    */
    Countly.remove_consent = function(feature) {
        cc.log("Removing consent for " + feature);
    ...
    */
    function add_cly_events(event) {
        if (!event.key) {
            cc.log("Event must have key property");
            return;
        }
        if (cluster.isMaster) {
            if (!event.count) {
    ...
    **/
    Countly.start_event = function(key) {
        if (timedEvents[key]) {
            cc.log("Timed event with key " + key + " already started");
            return;
        }
    ...
    **/
    Countly.end_event = function(event) {
        ...
        if (!timedEvents[event.key]) {
            cc.log("Timed event with key " + event.key + " was not started");
            return;
        }
    ...
    **/
    Countly.user_details = function(user) {
        if (Countly.check_consent("users")) {
            cc.log("Adding userdetails: ", user);
    ...
    Countly.report_feedback = function(feedback) {
        if (Countly.check_consent("star-rating") || Countly.check_consent("feedback")) {
            if (!feedback.widget_id) {
                cc.log("Feedback must contain widget_id property");
                return;
            }
            if (!feedback.rating) {
                cc.log("Feedback must contain rating property");
                return;
            }
            if (Countly.check_consent("events")) {
                ...
                cc.log("Reporting feedback: ", event);
                Countly.add_event(event);
            }
        }
    };
    ...
    **/
    Countly.fetch_remote_config = function(keys, omit_keys, callback) {
        if (Countly.check_consent("remote-config")) {
            ...
            cc.log("Remote config requires explicit consent");
            if (typeof callback === "function") {
        ...
        if (Countly.check_consent("apm")) {
            ...
            for (var i = 0; i < props.length; i++) {
                if (props[i] !== "apm_attr" && typeof trace[props[i]] === "undefined") {
                    cc.log("APM trace must have a", props[i]);
                    return;
                }
            }
    ...
    **/
    Countly.request = function(request) {
        if (!request.app_key || !request.device_id) {
            cc.log("app_key or device_id is missing");
            return;
        }
        if (cluster.isMaster) {
